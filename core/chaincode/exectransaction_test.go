...
// Check the correctness of the final state after N transaction execution.
func checkFinalStateN(chaincodeID string, N int) error {
	// Check the state in the ledger
	ledgerObj, ledgerErr := ledger.GetLedger()
	if ledgerErr != nil {
		return fmt.Errorf("Error checking ledger for <%s>: %s", chaincodeID, ledgerErr)
...
func invokeExample02TransactionN(ctxt context.Context, cID *pb.ChaincodeID, args []string, destroyImage bool, N int) error {
...
func TestExecuteInvokeTransaction2(t *testing.T) {
	var opts []grpc.ServerOption
	...
	args := []string{"a", "b", "10"}
	err = invokeExample02TransactionN(ctxt, chaincodeID, args, true, 10000)
	if err != nil {
		t.Fail()
		t.Logf("Error invoking transaction: %s", err)
	} else {
...
func BenchmarkExecuteInvokeTransaction(b *testing.B) {
	var opts []grpc.ServerOption
	...
	if err != nil {
		b.Fail()
		b.Logf("Error starting peer listener %s", err)
		return
	}
	...
	if err != nil {
		b.Fail()
		b.Logf("Error invoking transaction: %s", err)
	} else {
		fmt.Printf("Invoke test passed\n")
		b.Logf("Invoke test passed")
	}
